...
下载 \TeX~Live 的
\href{https://mirrors.ctan.org/systems/texlive/Images/texlive2026.iso}{\texttt{iso} 镜像文件},
...
\begin{lstlisting}
  cd /d X:\Y
  certutil -hashfile texlive2026.iso md5
\end{lstlisting}
%\begin{lstlisting}
%  (Get-FileHash 'X:\Y\texlive2026.iso' -Algorithm MD5).Hash
%\end{lstlisting}
若系统显示
\begin{lstlisting}[language = {}]
  MD5 的 texlive2026.iso 哈希:
  f5872cb2dec838670f91ed5c62493553
...
\begin{lstlisting}
  cd /d X:\Y
  certutil -hashfile texlive2026.iso sha512
\end{lstlisting}
%\begin{lstlisting}
%  (Get-FileHash 'X:\Y\texlive2026.iso' -Algorithm SHA512).Hash
%\end{lstlisting}
若系统显示
\begin{lstlisting}[literate={a}{a}{1} {b}{b}{1} {c}{c}{1} {d}{d}{1} {e}{e}{1} {f}{f}{1}]
  SHA512 的 texlive2026.iso 哈希:
...
\begin{lstlisting}[language = {}, deleteemph = set]
  ...
  = help>   https://tug.org/texlive/doc/install-tl.html   <=======
   ...
   <C> set installation collections:
       41 collections out of 41, disk space required: 9615 MB (free: 203277 MB)
...
这时一定要关注其中的
\begin{lstlisting}[language = {}, deleteemph = set]
  disk space required: 9615 MB (free: 203277 MB)
\end{lstlisting}
它代表了安装所需空间 "9615 MB",
而目前空余空间 "203277 MB".
根据每台电脑的实际情况,
...
安装才能顺利.
确定空余空间大于安装所需空间后,
用户可直接按键盘 \keys{I} 在默认路径中直接安装 \TeX~Live,
...
\begin{lstlisting}[language = {}]
  ...
  <1> TEXDIR:       C:/texlive/2026
      main tree:    C:/texlive/2026/texmf-dist
  
  <2> TEXMFLOCAL:     C:/texlive/texmf-local
  <3> TEXMFSYSVAR:    C:/texlive/2026/texmf-var
  <4> TEXMFSYSCONFIG: C:/texlive/2026/texmf-config
  
  <5> TEXMFVAR:       ~/.texlive2026/texmf-var
  <6> TEXMFCONFIG:    ~/.texlive2026/texmf-config
  <7> TEXMFHOME:      ~/texmf
...
这时按键盘 \keys{1}, 将看到
\begin{lstlisting}[language = {}]
  New value for TEXDIR [C:/texlive/2026]:
\end{lstlisting}
这时可更改路径, 如 \path{D:/texlive/2026}.
...
\begin{lstlisting}[escapechar = |]
  wget |\url{http://mirrors.ctan.org/systems/texlive/Images/texlive2026.iso}|
\end{lstlisting}
%\begin{lstlisting}
%  Invoke-WebRequest -Uri 'http://mirrors.ctan.org/systems/texlive/Images/texlive2026.iso' -OutFile 'texlive2026.iso'
%\end{lstlisting}
即可下载最近的镜像.
类似地,
也可以指定国内的源进行下载,
例如选择清华大学镜像
\begin{lstlisting}[escapechar = |]
  wget |\url{https://mirrors.tuna.tsinghua.edu.cn/CTAN/systems/texlive/Images/texlive2026.iso}|
\end{lstlisting}
%\begin{lstlisting}
%  Invoke-WebRequest -Uri 'https://mirrors.tuna.tsinghua.edu.cn/CTAN/systems/texlive/Images/texlive2026.iso' -OutFile 'texlive2026.iso'
%\end{lstlisting}
其他国内镜像只需要将链接替换即可,
...
\begin{lstlisting}
  C:\texlive\2026\tlpkg\installer\uninst.bat
\end{lstlisting}
%\begin{lstlisting}
%  & 'C:\texlive\2026\tlpkg\installer\uninst.bat'
%  Remove-Item -Recurse -Force -Path '$env:USERPROFILE\.texlive2026'
%\end{lstlisting}
结束以上步骤后,
手动删除用户文件夹中的 \texttt{.texlive2026}.
...
用户需要清理注册表
\begin{lstlisting}[language = {}]
  HKEY_CURRENT_USER\Software\Classes\TL.DVIOUT.view.2026
  HKEY_CURRENT_USER\Software\Classes\TL.TeXworks.edit.2026
  HKEY_CURRENT_USER\Software\Classes\TL.PSViewer.view.2026
  HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\ApplicationAssociationToasts\TL.TeXworks.edit.2026_.tex
\end{lstlisting}
但若在安装时使用管理员权限``为所有人安装'',
则注册表位置将变为
\begin{lstlisting}[language = {}]
  HKEY_LOCAL_MACHINE\SOFTWARE\Classes\TL.DVIOUT.view.2026
  HKEY_LOCAL_MACHINE\SOFTWARE\Classes\TL.TeXworks.edit.2026
  HKEY_LOCAL_MACHINE\SOFTWARE\Classes\TL.PSViewer.view.2026
\end{lstlisting}
