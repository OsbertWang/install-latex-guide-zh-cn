...
\textbf{根据以往经验, 安装老版 C\TeX\ 套装的用户往往会有} \path{system32} \textbf{丢失的问题}. 
...
\end{lstlisting}
即可看到如下结果
\begin{lstlisting}[keywordstyle = {}, emphstyle = {}, stringstyle = {}]
...
按 \keys{D} 后, 用户可看到
\begin{lstlisting}[deletekeywords = {return, command}]
  ==============================================================================
...
\end{lstlisting}
显示
\begin{lstlisting}[deletekeywords = source]
...
\begin{lstlisting}[deletekeywords = tex]
  ...
  notepad main.tex
\end{lstlisting}
第1行表示创建一个工作路径 \path{D:\work-latex},
...
该值即为 \TeX\ Live 的安装路径%
\footnote{%
  除这里提供的 \path{TEXMFROOT}, 还有很多其他变量, 可在
