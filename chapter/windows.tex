...
\texttt{Y} 表示文件夹;
默认路径名称为不带空格的英文}.
下载完毕后, 用户打开 \textsf{PowerShell} 窗口%
...
通常在开启 \textsf{PowerShell} 或上一程序运行结束时会显示.

在 \textsf{PowerShell} 执行
\begin{lstlisting}
  $env:Path -split ';'
\end{lstlisting}
查看环境变量. 
...
至此, 用户只需耐心等待安装完成, 并且不要点击 \textsf{PowerShell} 窗口%
\footnote{Windows 11 默认采取快速编辑模式, 一旦点击 \textsf{PowerShell} 窗口,
将显示``选择命令提示符''而导致进程暂停}. 
安装完成后, 关闭再打开 \textsf{PowerShell} 窗口%
\footnote{正常安装 \TeX{} Live 后, 环境变量会变化,
关闭原 \textsf{PowerShell} 窗口再重新打开才能获取新的环境变量},
执行
\begin{lstlisting}
  tex -v
\end{lstlisting}
...
如果有些用户使用某些浏览器时无法正常下载镜像文件.
用户可以在 \textsf{PowerShell} 执行
\begin{lstlisting}
  iwr -Uri 'http://mirrors.ctan.org/systems/texlive/Images/texlive2025.iso' -OutFile 'texlive2025.iso'
\end{lstlisting}
即可下载最近的镜像.
类似地,
也可以指定国内的源进行下载,
例如选择清华大学镜像
\begin{lstlisting}
  iwr -Uri 'https://mirrors.tuna.tsinghua.edu.cn/CTAN/systems/texlive/Images/texlive2025.iso' -OutFile 'texlive2025.iso'
\end{lstlisting}
其他国内镜像只需要将链接替换即可,
...
有些用户反馈安装过程中需要管理员权限.
以管理员身份打开 \textsf{PowerShell} 窗口的方法是点击 \keys{\faWindows},
输入 \textsf{PowerShell},
按 \keys{\ctrl + \shift + \enter} 三个键.
一般而言,
在系统盘内直接安装 \TeX{} Live 会碰到此类问题,
因此鼓励用户在安装前先手动建立安装路径.
直接使用管理员身份打开 \textsf{PowerShell} 窗口进行安装可能会对以后的升级造成影响.
...
在 \textsf{PowerShell} 中执行
\begin{lstlisting}
  ni -ItemType Directory -Path 'C:\temp' -Force
  $env:TEMP = 'C:\temp'
  $env:TMP = 'C:\temp'
\end{lstlisting}
...
下面将介绍使用命令行升级宏包的方法. 
打开 \textsf{PowerShell} 窗口, 首先执行下面命令指定升级使用的镜像源. 
...
因此在这里我只介绍从源处安装宏包的命令. 
假设用户想安装 \texttt{mcmthesis} 宏包, 只需在 \textsf{PowerShell} 执行
\begin{lstlisting}
...
接下来,
用户只需执行%
\footnote{双击或在 \texttt{PowerShell} 中运行}%
安装路径中的卸载批处理文件即可实现卸载, 如默认安装时,
依次执行
\begin{lstlisting}
  & 'C:\texlive\2025\tlpkg\installer\uninst.bat'
  ri -Recurse -Force -Path '$env:USERPROFILE\.texlive2025'
\end{lstlisting}
