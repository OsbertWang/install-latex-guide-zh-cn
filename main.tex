...
\subsubsection{显示行号}
打开\texttt{编辑}菜单\texttt{首选项}的\texttt{编辑器}标签，选择\texttt{行号}即可。

\subsubsection{自动补全}
自动补全主要是 \texttt{tab} 键的功能，使用前需确定\texttt{首选项}的\texttt{编辑器}标签中是否勾选了\texttt{启动自动补全}。

\subsubsection{使用模板}
TeXworks支持从模板新建文档，在\textbf{文件}菜单中可见。

\subsubsection{拼写检查}
默认情况，Texworks没有搭载拼写检查字典，需要用户自己配置。
首先，访问 \href{http://wiki.openoffice.org/wiki/Dictionaries}{openoffice} 下载词典。接下来将词典安装到 \texttt{<resources>\textbackslash dictionaries}。\texttt{<resources>} 的具体位置可以在\texttt{帮助}的\texttt{TeXworks 配置与资源}中找到。安装完成后，在\texttt{编辑}菜单的\texttt{拼写}中选择即可。
...
\end{lstlisting}
表示该文档使用 \texttt{<language\_code>} 进行拼写检查，它的具体名称请用户到\texttt{编辑}菜单的\texttt{拼写}中查看。
