...
安装完成后, 用户需要设置环境变量.
继续在 WSL 中执行\footnote{这里使用了 vim, 如果用户不习惯使用它, 也可在主系统中通过 \href{https://www.voidtools.com/zh-cn/}{everything} 搜索文件 \texttt{.bashrc}, 找到路径在 \menu{... > rootfs > home > username} 的文件, 使用编辑器打开并进行更改}
\begin{lstlisting}[language = bash]
  sudo vim ~/.bashrc
\end{lstlisting}
在打开的文件末尾添加
