...
Ubuntu 源内自带了 \TeX{} Live, 因此用户可以在 \textsf{Terminal}\footnote{点击鼠标右键即可看到打开 Terminal 窗口的选项} 中使用以下命令进行安装源内的完整版
\begin{lstlisting}[language=sh]
  sudo apt-get install texlive-full
\end{lstlisting}
在这里, 我并不反对用户使用源内的版本, 但这并不是本文的核心内容.

\subsection{用镜像安装}

为使用户顺利使用 \TeX{} Live 2019, 建议用户首先卸载从源内安装的 \TeX{} Live.

下载 \href{http://mirrors.ctan.org/systems/texlive/Images/texlive.iso}{iso 镜像文件} 的方法如前所述.
下载完毕后, 打开 \textsf{Terminal} 窗口, 使用如下代码加载光盘镜像至 \texttt{/mnt/texlive} 文件夹
\begin{lstlisting}
  mkdir /mnt/texlive
...
如果是从源内安装, 在 \textsf{terminal} 中输入
\begin{lstlisting}
  sudo apt autoremove texlive-*
\end{lstlisting}
进行卸载, 其中 \texttt{*} 代表源中的 \TeX{} Live 包.

如果是从光盘镜像安装, 只需要直接删除 \texttt{/usr/local/texlive/2019} 文件夹即可
\begin{lstlisting}
  sudo rm -r /usr/local/texlive/2019
\end{lstlisting}
当然还要清理掉环境变量.
